...
app.get('/', (req, res) => {
  res.send('Bot de Telegram encendido ✅');
});
...
if (!TOKEN) {
  console.error('❌ Falta configurar TOKEN');
  process.exit(1);
}
...
async function downloadTikTok(chatId, url) {
  ...
  if (!data) throw new Error('TikTok no devolvió datos');
  ...
  try {
    ...
    if (sizeMB > 49) {
      return bot.sendMessage(
        chatId,
        `❌ El video pesa ${sizeMB.toFixed(1)} MB y supera el límite de Telegram.`
      );
    }
  ...
  try {
    ...
    bot.sendMessage(
      ...
      '❌ No pude descargar ese contenido.\n\n' +
        'Posibles causas: cuenta privada, video eliminado, link no público o bloqueo de la plataforma.'
    );
  }
});